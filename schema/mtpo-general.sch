...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2"
    ...
    <sch:pattern>
        ...
        <sch:rule context="tei:TEI">
            <sch:assert id="general-0002.02" test="@xml:id = descendant::tei:idno[@type = 'LOCAL']">The element <sch:name/> must have an xml:id attribute with its value equal to a descendant idno element with type = 'LOCAL'</sch:assert>
        </sch:rule>
    </sch:pattern>
    <sch:pattern>
        <sch:rule context="tei:publicationStmt">
            <sch:assert id="pubSt-0001" test="child::tei:date/@type = 'sort'">The <sch:name/> element must have a child date element with a type attribute with value 'sort'</sch:assert>
            <sch:assert id="pubSt-0002" test="child::tei:date/@type = 'pub'">The <sch:name/> element must have a child date element with a type attribute with value 'pub'</sch:assert>
        </sch:rule>
    </sch:pattern>
</sch:schema>